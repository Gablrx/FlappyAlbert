...
const render = () => {
    ...
    if (gamePlaying) {
        pipes.map(pipe => {
            ...
            // top 
            ctx.drawImage(img, 432, 768 - pipe[1], pipeWidth, pipe[1], pipe[0], 0, pipeWidth, pipe[1]);
            // bottom 
            ctx.drawImage(img, 432 + pipeWidth, 288, pipeWidth, canvas.height - pipe[1] + pipeGap, pipe[0], pipe[1] + pipeGap, pipeWidth, canvas.height - pipe[1] + pipeGap);
